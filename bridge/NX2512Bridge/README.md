...
Before loading the DLL through `File > Execute > NX Open`, set a random `DSH_NX_BRIDGE_TOKEN` and optional `DSH_NX_BRIDGE_PORT`. Configure the MCP side with the same token and `DSH_NX_BRIDGE_URL=http://127.0.0.1:48161/`.
